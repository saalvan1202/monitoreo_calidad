...
const WaterTanks = () => {
  const { idZone, setIdZone, idArea, setIdArea, zones, areas, tanks } =
    useMonitoring();
              ...
              <div className="cartas__caja-cartas__info-cards w-full gap-20">
                {tanks.map((tank) => (
                  <InfoCards
                    ...
                    id={tank.id}
                    nombre={tank.name}
                    descripcion={tank.description}
                    type_card={"tank"}
                    color_card={YELLOW_CARD}
                    formRef={formReftEdit}
                  >
                    <FormWaterTanks
                      formRef={formReftEdit}
                      handleSubmit={(value) => handleSubmitEdit(value, tank.id)}
                      id_area={idArea}
                      nombre={tank.name}
                      descripcion={tank.description}
                    />
